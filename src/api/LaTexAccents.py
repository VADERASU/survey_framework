...
# from https://github.com/bwhelm/LaTex-handler/tree/master
import re
...
class AccentConverter:
    def __init__(self):
        (
            ...
            self.accent_detector,
        ) = self.__create_translation_rules()
    ...
    def __create_translation_rules(self):
        ...
        encode_dict = (
            {}
        )  # each key is an accented Ascii char (without blank space), its corresponding value is its UTF8 version
        regex_detectors = (
            []
        )  # each item is a regex detecting a given Latex accent pattern
        ...
        self.populate_encode_dict(
            encode_dict, strKey_umlaut, strValue_umlaut, '{\\"', "}"
        )
        ...
        )  # backslash followed by " followed by any number of spaces followed by a char
        self.populate_encode_dict(
            encode_dict, strKey_umlaut, strValue_umlaut, '\\"{', "}"
        )
        regex_detectors.append(re.compile(r'\\"{ *[a-zA-Z]{1} *}'))
        ...
        self.populate_encode_dict(
            encode_dict, strKey_acute, strValue_acute, "{\\'", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\' *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_acute, strValue_acute, "\\'", ""
        ...
        self.populate_encode_dict(
            encode_dict, strKey_hungarian, strValue_hungarian, "\\H{", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_grave, strValue_grave, "{\\`", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\` *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_grave, strValue_grave, "\\`", ""
        ...
        self.populate_encode_dict(
            encode_dict, strKey_circumflex, strValue_circumflex, "{\\^", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\\^ *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_circumflex, strValue_circumflex, "\\\^", ""
        ...
        self.populate_encode_dict(
            encode_dict, strKey_hraceck, strValue_hraceck, "{\\v", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_breve, strValue_breve, "\\u{", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_cedilla, strValue_cedilla, "{\\c", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_dot, strValue_dot, "{\\\.", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\\. *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_dot, strValue_dot, "\\.", ""
        ...
        self.populate_encode_dict(
            encode_dict, strKey_dot_under, strValue_dot_under, "\\d{", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_ogonek, strValue_ogonek, "\\k{", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_tilde, strValue_tilde, "{\\~", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\~ *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_tilde, strValue_tilde, "\\~", ""
        ...
        self.populate_encode_dict(
            encode_dict, strKey_macron, strValue_macron, "{\\=", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\= *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_macron, strValue_macron, "\\=", ""
        ...
        self.populate_encode_dict(
            encode_dict, strKey_bar_under, strValue_bar_under, "\\b{", "}"
        )
        ...
        self.populate_encode_dict(
            encode_dict, strKey_ring, strValue_ring, "\\r{", "}"
        )
        regex_detectors.append(re.compile(r"\\r{ *[a-zA-Z]{1} *}"))
        self.populate_encode_dict(
            encode_dict, strKey_ring, strValue_ring, "\\r\\", ""
        )
        regex_detectors.append(re.compile(r"\\r *\\[a-zA-Z]{1} *"))
        ...
        strKey_s = ""
        self.populate_encode_dict(
                encode_dict, strKey_s, strValue_s, "{\\ss", "}"
        )
        regex_detectors.append(re.compile(r"{ *\\ss *}"))
        return encode_dict, regex_detectors

    def populate_encode_dict(
        self,
        encode_dict,
        strKey,
        strValue,
        accent_pattern_left,
        accent_pattern_right="",
    ):
        """
        ...
        for i, charKey in enumerate(s_key):
            encode_dict[
                accent_pattern_left + charKey + accent_pattern_right
            ] = (s_value[i], charKey)
    ...
    def decode_Tex_Accents(self, s, utf8_or_ascii=1):
        ...
        for i in range(len(self.accent_detector)):
            accent_pattern = self.accent_detector[
                i
            ]  # this is a complied regex corresponding to some accent pattern

            m = set(
                re.findall(accent_pattern, s)
